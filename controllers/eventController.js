...
const { EventWbenificiary, Project, Beneficiary } = require('../models');
...
exports.submitEvent = async (req, res) => {
  try {
    ...
    console.log('GeoJSON Location:', JSON.stringify({
      type: 'Point',
    ...
    // Create the event with a geographic location
    const eventWithBeneficiary = await sequelize.transaction(async (t) => {
      const event = await EventWbenificiary.create({
        eventName,
        outcome,
        eventType,
        startDate,
        ...
        photographs,
        reports,
        location: { // Constructing location object here
          type: 'Point',
          coordinates: [lon, lat]
        },
        projectId
      ...
      const beneficiaries = cleanedBeneficiaries.map(b => {
        const provinceCode = province.substring(0, 3).toUpperCase();
        const districtCode = district.substring(0, 3).toUpperCase();
        const randomPart = randomBytes(3).toString('hex').toUpperCase();
        const uniqueCode = `${provinceCode}-${districtCode}-${randomPart}`;
        return { ...b, eventId: event.id, uniqueCode };
      });
      const createdBeneficiaries = await Beneficiary.bulkCreate(beneficiaries, { transaction: t });
      await event.addBeneficiaries(createdBeneficiaries, { transaction: t });
    ...
    await project.addEvent(eventWithBeneficiary);
    await project.save();
...
exports.showEditForm = async (req, res) => {
  try {
    const projectId = req.params.projectId;
    const project = await Project.findByPk(projectId);  
    const event = await EventWbenificiary.findByPk(req.params.id, { include: [Beneficiary] });
    if (!event) {
...
exports.updateEvent = async (req, res) => {
  try {
    ...
    const event = await EventWbenificiary.findByPk(eventId);
    if (!event) {
...
exports.deleteEvent = async (req, res) => {
  try {
    ...
    const project = await Project.findByPk(event.projectId);
    if (project) {
      await project.removeEvent(event);
    }
...
exports.viewAllEventData = async (req, res) => {
  try {
    ...
    if (outcome && outcome !== 'undefined') {query.outcome = { [Op.contains]: [outcome] };}
    ...
    if (startDate && startDate !== 'undefined' && !isNaN(new Date(startDate).getTime())) {
      query.startDate = { [sequelize.Op.gte]: new Date(startDate) };
    }
    if (endDate && endDate !== 'undefined' && !isNaN(new Date(endDate).getTime())) {
      query.endDate = { [sequelize.Op.lte]: new Date(endDate) };
    }
    let events = await EventWbenificiary.findAll({ where: query, include: [{ model: Beneficiary, as: 'beneficiaries' }] });
    ...
    const allEvents = await EventWbenificiary.findAll({ where: { projectId: project.id } });
    const uniqueOutcomes = [...new Set(allEvents.map(e => e.outcome).flat().filter(Boolean))];
    const uniqueActivities = [...new Set(allEvents.map(e => e.eventName).filter(Boolean))];
    const uniqueMunicipalities = [...new Set(allEvents.map(e => e.venue?.municipality).filter(Boolean))];
    ...
    if (sort === 'asc') {
      ...
      events = events.sort((a, b) => {
        return overview.find(o => o.eventId.toString() === b.id.toString()).totalAttendees -
               overview.find(o => o.eventId.toString() === a.id.toString()).totalAttendees;
      });
    } else if (sort === 'lowest_attendee') {
      events = events.sort((a, b) => {
        return overview.find(o => o.eventId.toString() === a.id.toString()).totalAttendees -
               overview.find(o => o.eventId.toString() === b.id.toString()).totalAttendees;
      });
    }
    ...
    events.forEach(event => {
      ...
      const totalDisability = event.beneficiaries ? event.beneficiaries.filter(b => b.disability).length : 0;
...
exports.viewOneEventData = async (req, res) => {
  try {
    // Modify the findByPk query to include the location as hex-encoded WKB
    const event = await EventWbenificiary.findByPk(req.params.id, {
      include: [{ model: Beneficiary, as: 'beneficiaries' }],
      attributes: {
        include: [
          [sequelize.fn('ST_AsHexEWKB', sequelize.col('location')), 'location_wkb_hex']
        ]
      }
    });
    if (!event) {
    ...
    // Check if the request is for exporting to Excel (no changes needed here)
    if (req.query.export === 'excel') {
      ...
      summarySheet.addRows(summarySheetData);
...
exports.viewEventData = async (req, res) => {
  try {
    const datas = await EventWbenificiary.findAll({ include: [Beneficiary] });
